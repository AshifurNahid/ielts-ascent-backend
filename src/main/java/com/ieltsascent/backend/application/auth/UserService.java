...
public class UserService {
    ...
    public User updateTargetBand(UUID userId, double targetBand) {
        User user = getUser(userId);
        user.getProfile().setTargetBand(targetBand);
        return userRepository.save(user);
    }

    public User updateBaseline(UUID userId, double currentBand, double targetBand) {
        User user = getUser(userId);
    ...
    public User updateExamDate(UUID userId, LocalDate examDate) {
        User user = getUser(userId);
        user.getProfile().setExamDate(examDate);
        return userRepository.save(user);
    }

    public User updateOnboarding(UUID userId, boolean completed, UserProfile.StudyPreference studyPreference) {
        User user = getUser(userId);
    ...
    public ProfileReadiness resolveReadiness(UUID userId) {
        User user = getUser(userId);
